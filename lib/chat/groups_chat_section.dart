...
class GroupChatSection extends StatefulWidget {
  ...
  const GroupChatSection({super.key, required this.groupId, required this.groupName, required this.memberList});
...
class GroupChatSectionState extends State<GroupChatSection> {
  ...
  Future<List<GroupMessageModel>> getData() async {
    if (jwtToken != null) {
      final results = await _apiChat.getGroupMessage(jwtToken!, widget.groupId.toString());
  ...
  void refreshGroupList() {
    setState(() {
      ...
      if(jwtToken != null){
        futureApiGroupList = _apiChat.getGroupNameList(jwtToken!); // Re-fetch the group list
      }
    ...
    if (_controller.text.trim().isNotEmpty) {
      setState(() {
        messages.add(GroupMessageModel(
          ...
          senderName: "Me", // Replace with actual sender name
    ...
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          ...
          title: GestureDetector(
            onTap: () {
             showDialog(
                 context: context,
                 builder: (context){
                   return MyDialog(refreshGroupListCallback:  refreshGroupList,groupName: widget.groupName,memberList: widget.memberList, groupId: widget.groupId,);
                 });
            },
            child: const Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("BrightLink", style: TextStyle(color: Colors.black, fontSize: 20)),
                Text('Abiha, Umme, Kartik, Kavita, +2', style: TextStyle(color: Color(0xff2354C5), fontSize: 13)),
              ],
            ),
          ),
          leadingWidth: 90,
          leading: Row(
            children: [
              IconButton(
                onPressed: () {
                  Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const GroupsName()));
                },
                icon: const Icon(Icons.arrow_back, color: Colors.blue),
              ),
              SizedBox(width: 40, child: Image.asset('assets/images/bright.png')),
            ],
          ),
          actions: [
            PopupMenuButton(
              ...
              onSelected: (value) {
                switch (value) {
                  case 'Group Info':
                    Navigator.push(context,MaterialPageRoute(builder: ((_)=>GroupMemberInfo())));
                    break;
        ...
        bottomNavigationBar: Container(
          decoration: const BoxDecoration(gradient: LinearGradient(colors: [blueBgColor, blueBgColor1])),
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if(senderImage != null && senderImage!.isNotEmpty){
                            try{
                              bytes = base64Decode(senderImage!.replaceAll(RegExp(r'\s+'), ''));
                            }catch(e){
                              bytes = Uint8List(0);
                            }
                          }else{
                            ...
                            child: Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Padding(
                                    padding: isMe ? const EdgeInsets.only(right: 25) : const EdgeInsets.only(left: 25),
                                    child: Container(
                                      padding: const EdgeInsets.all(12.0),
                                      decoration: BoxDecoration(
                                        color: isMe ? const Color(0xff2354C5) : const Color(0xffF5F8FF),
                                        border: Border.all(color: isMe ? const Color(0xff2354C5) : const Color(0xff2354C5)),
                                        borderRadius: isMe
                  ...
                  decoration: InputDecoration(
                    suffixIcon: GestureDetector(
                      onTap: () async {
                        ...
                        if(jwtToken != null){
                          await _apiSendMessage.sendGroupMessage(jwtToken!, content, senderName!, senderId!, groupId, timeStamp);
...
class _MyDialogState extends State<MyDialog> {
  ...
  Set<String> groupMember = {}; // Ensure this is a Set<String> to handle member IDs
  String? jwtToken;
  ...
  void initState() {
    ...
    _loadJwtToken().then((_) {
      setState(() {
        ...
        groupMember = widget.memberList.split(',').toSet(); // Initialize with existing members
      });
    });
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: TextField(
                            ...
                            onChanged: (value) {
                              setState(() {}); // Refresh the list based on search query
                            },
                            decoration: InputDecoration(
                          ...
                          child: FutureBuilder(
                            ...
                            builder: (context, snapshot) {
                              if (snapshot.connectionState == ConnectionState.waiting) {
                                ...
                                if (tracker.isEmpty || tracker.length != userList.length) {
                                  tracker = List.generate(userList.length, (index) =>
                                      groupMember.contains(userList[index].cId.toString()));
                                }

                                return ListView.builder(
                                  itemCount: filteredList.length,
                                  itemBuilder: (BuildContext context, int index) {
                                    String base64Image = '${filteredList[index].images}';
                                      ...
                                      children: [
                                        SizedBox(
                                          ...
                                          child: ListTile(
                                            ...
                                            trailing: Checkbox(
                                              value: tracker[userList.indexOf(filteredList[index])],
                                              onChanged: isDialogDisabled ? null : (bool? value) {
                                                setState(() {
                                                  tracker[userList.indexOf(filteredList[index])] = value ?? false;
                                                  if (value ?? false) {
                                                    groupMember.add(filteredList[index].cId.toString());
                                                  } else {
                                                    groupMember.remove(filteredList[index].cId.toString());
                                                  }
                                                });
                                              },
                                            ),
                          ...
                          child: Center(
                            child: GestureDetector(
                              onTap: isDialogDisabled
                                  ...
                                  : () async {
                                setState(() {
                                  isDialogDisabled = true; // Disable the dialog while creating the group
                                });

                                var groupName = groupNameController.text.trim(); // Get the trimmed group name
                                if (groupName.isNotEmpty) {
                                  try {
                                    // Ensure groupMember is not null before joining
                                    String memberListString = groupMember.where((member) => member != null).join(',');
                                    ...
                                    setState(() {
                                      isDialogDisabled = false; // Re-enable the dialog
                                    });
                                  }
                                } else {
